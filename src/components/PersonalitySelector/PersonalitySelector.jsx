...
function PersonaCard({ persona, index, selected, onSelect }) {
  ...
  return (
    <motion.button
      ...
      whileHover={!isSelected ? { scale: isDimmed ? 0.99 : 1.01 } : {}}
      style={{
        all: 'unset',
        cursor: 'pointer',
        position: 'relative',
        padding: '22px 20px',
        background: isSelected ? 'rgba(201,168,76,0.06)' : '#141b34',
        border: isSelected
    ...
    >
      ...
      <div style={{
        ...
        <span style={{ color: persona.accent }}>
          <PersonaGlyph id={persona.id} size={44} />
        </span>
        <span style={{
          fontFamily: '"JetBrains Mono", ui-monospace, monospace',
          fontSize: 9,
      ...
      {/* Tagline — uses persona's own accent color */}
      <p style={{
